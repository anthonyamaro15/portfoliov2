...
// Dynamic imports for Three.js components (requires WebGL)
const ParticleField = dynamic(() => import('@/components/ui/particle-field').then((mod) => mod.ParticleField), {
  ssr: false,
});

const MorphingGeometry = dynamic(
  () => import('@/components/ui/morphing-geometry').then((mod) => mod.MorphingGeometry),
  { ssr: false }
);

const techBadges = ['4+ years shipping', 'React / Node / Rust', 'Vercel / AWS / Azure'];
...
const FloatingCode = ({
  ...
  const snippets = {
    default: (
      <>
        <span className='text-emerald-400'>const</span> build = <span className='text-amber-400'>()</span> =&gt; ...
      </>
    ),
    rust: (
      <>
        <span className='text-rose-400'>fn</span> <span className='text-blue-400'>main</span>
        <span className='text-amber-400'>()</span> {'{'} ... {'}'}
      </>
    ),
    react: (
      <>
        <span className='text-cyan-400'>&lt;App</span> <span className='text-amber-400'>/&gt;</span>
      </>
    ),
  };
...
export const Hero = () => {
  ...
  return (
    ...
    >
      {/* Three.js Particle Field Background - using fixed height to ensure Canvas gets proper dimensions */}
      <div className='absolute inset-0 -z-10' style={{ minHeight: '90vh' }}>
        <ParticleField />
      </div>

      {/* Three.js Morphing Geometry */}
      {/* Available shapes: "icosahedron" | "torus" | "octahedron" | "pointSphere" | "pointCube" | "torusKnot" | "dnaHelix" | "matrixGrid" | "dodecahedron" */}
      <div className='absolute inset-0 -z-10 opacity-60' style={{ minHeight: '90vh' }}>
        <MorphingGeometry shapes={['matrixGrid']} />
      </div>
      ...
      <FloatingCode delay={0} className='absolute top-32 left-[8%] hidden lg:block' variant='default' />
      <FloatingCode delay={2} className='absolute top-48 right-[12%] hidden lg:block' variant='rust' />
      <FloatingCode delay={3.5} className='absolute bottom-48 right-[6%] hidden xl:block' variant='react' />

      {/* Geometric shapes */}
      <FloatingElement delay={1} className='absolute bottom-40 left-[18%] hidden lg:block' yOffset={8}>
        <div className='w-14 h-14 border border-border/30 rounded-bento rotate-12' />
      </FloatingElement>
      <FloatingElement delay={3} className='absolute top-60 right-[8%] hidden lg:block' yOffset={6}>
        <div className='w-4 h-4 bg-foreground/10 rounded-full' />
      </FloatingElement>

      {/* Additional shapes for visual depth */}
      <FloatingElement delay={2.5} className='absolute top-36 right-[22%] hidden xl:block' yOffset={10}>
        <div className='w-6 h-6 border border-border/25 rotate-45' />
      </FloatingElement>
      <FloatingElement delay={1.5} className='absolute bottom-52 left-[10%] hidden lg:block' yOffset={6}>
        <div className='w-3 h-3 bg-foreground/8 rotate-45' />
      </FloatingElement>
      <FloatingElement delay={4} className='absolute top-72 left-[15%] hidden xl:block' yOffset={8}>
        <div className='w-8 h-8 border border-border/20 rounded-full' />
      </FloatingElement>
      <FloatingElement delay={2} className='absolute bottom-32 right-[18%] hidden lg:block' yOffset={12}>
      ...
      <div className='mx-auto max-w-[1100px] px-5 md:px-8 text-center'>
        ...
        >
          I build high-performance web systems end-to-end — React/Next.js on the front, Node/Postgres on the back, and
          Rust where latency matters.
        </motion.p>
